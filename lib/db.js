...
exports.createSearchTable = function(cb){
...
exports.fullSearchReIndex = function(callback){
		...
		client.query("TRUNCATE search", function(err, result) {
			...
			stream.pipe(es.map(function (data, cb) {
				...
				if (config['ignoreSearchClassmarkPrefixes'].indexOf(firstTwo) === -1 && config['ignoreSearchClassmarkPrefixes'].indexOf(firstThree) === -1 && config['ignoreSearchClassmarkPrefixes'].indexOf(firstFour) === -1){
					...
					exports.returnClassmarkHierarchy(classmark,function(hierarchy){
						
						//add it to the search table
						var label = hierarchy.join(" > ");

						//get the hidden label first
						exports.returnClassmark(classmark,function(err,classmarkData){
							...
							var hiddenLabel = exports.returnFieldFromRows('skos:hiddenLabel',classmarkData);
							...
							pg.connect(process.env.DATABASE_URL, function(err2, client2, done2) {
								...
								var queryText = 'INSERT INTO search(subject, label, hiddenlabel) VALUES($1, $2, $3)'
								client2.query(queryText, [data.subject, label, hiddenLabel ], function(err, result) {
									counter++
...
//give it what field you want eg 'skos:label' and it will return that value for database results
exports.returnFieldFromRows = function(predicate, rows){
...
exports.returnClassmarkHierarchy = function(classmark,callback){
	var hierarchy = []
	var getParent = function(childClassmark){
		exports.returnClassmark(childClassmark,function(err,classmarkData){

			if (err) callback([]);
			
			var parent = exports.returnFieldFromRows('skos:broader',classmarkData);
			var label = exports.returnFieldFromRows('skos:prefLabel',classmarkData);
...
exports.dropSearchTable = function(cb){
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		if (err) cb(err);
...
exports.testSearchTable = function(cb){
	pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		client.query('SELECT * FROM search limit 1', function(err, result) {
			cb(err,result)
			done();
