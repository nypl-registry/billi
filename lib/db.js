...
module.exports =  function(app){
	...
	var db = {
		...
		//***Recall methods
		returnClassification: function(classification,cb){


			if (!classification){
				if (cb) cb("bad classification",false);
				return false;
			}
			...
			pg.connect(process.env.DATABASE_URL, function(err, client, done) {
				...
				classification = 'classification:' + classification.toString().toLowerCase().replace(/\s/g,'_');
		...
		returnClassmarkHierarchy: function(classmark,returnObj,callback){
			...
			var getParent = function(childClassmark){
				self.returnClassmark(childClassmark,function(err,classmarkData){
					...
					if (label[0]){
						if (returnObj){
							hierarchy.push( { subject: childClassmark, prefLabel : label[0], hiddenLabel: hiddenLabel[0] } );
						}else{
