...
module.exports =  function(app){
	...
	var db = {
				...
				client.query('SELECT * FROM search limit 1', function(err, result) {
					if (err) { 
						if (err.code === '42P01'){
							...
							createTableSql = createTableSql + '	subject	varchar(128) collate "C",';
							createTableSql = createTableSql + '	hiddenlabel	varchar(128),';
							createTableSql = createTableSql + '	classification varchar(128),';
				...
				client.query('SELECT * FROM triples limit 1', function(err, result) {
					if (err) { 
						if (err.code === '42P01'){
							...
							createTableSql = createTableSql + '	id serial primary key,';
							createTableSql = createTableSql + '	subject	varchar(128) collate "C",';
							createTableSql = createTableSql + '	predicate	varchar(128),';
							createTableSql = createTableSql + '	objectUri	varchar(512) collate "C",';
							createTableSql = createTableSql + '	objectLiteral	text,';
							createTableSql = createTableSql + '	literalDataType	varchar(128),';
		...
		//create the search table
		createImagesTable:  function(cb){

			pg.connect(process.env.DATABASE_URL, function(err, client, done) {
				client.query('SELECT * FROM images limit 1', function(err, result) {
					if (err) { 
						if (err.code === '42P01'){

							console.log("building images table")

							var createTableSql = '';


							createTableSql = createTableSql + 'CREATE TABLE images (';
							createTableSql = createTableSql + '	id serial primary key,';
							createTableSql = createTableSql + '	subject	varchar(128) collate "C",';
							createTableSql = createTableSql + '	image bytea';
							createTableSql = createTableSql + ')';

							client.query(createTableSql, function(err, result) {

								if (err) console.error("Could not create the images table!",err);

								client.query("Create Index on images ((lower(subject)))", function(err, result) {
									if (err) console.error("Could not create images subject index!",err);
										if (cb) cb(null,true)
										done();	
								});
							});
				...
				client.query("TRUNCATE search", function(err, result) {
					...
					stream.pipe(es.map(function (data, cb) {
						...
						if (config['ignoreSearchClassmarkPrefixes'].indexOf(firstTwo) === -1 && config['ignoreSearchClassmarkPrefixes'].indexOf(firstThree) === -1 && config['ignoreSearchClassmarkPrefixes'].indexOf(firstFour) === -1){
							...
							self.returnClassmarkHierarchy(classmark,false,function(hierarchy){
		...
		//classmark - just the hidden label classmark, so "*m" or "q1-10000" etc.
		...
		returnClassmarkHierarchy: function(classmark,returnObj,callback){
			var self = this;
			...
			var getParent = function(childClassmark){
				...
				self.returnClassmark(childClassmark,function(err,classmarkData){
					...
					childClassmark = (classmarkData[0]) ? classmarkData[0].subject : 'class:'+childClassmark;



					if (label[0]){
						if (returnObj){
							hierarchy.push( { subject: childClassmark, classmarkUrlSafe: childClassmark.split("class:")[1], prefLabel : label[0], hiddenLabel: hiddenLabel[0] } );
						}else{
							hierarchy.push(label[0]);
						}
					}else{
						if (returnObj){
							hierarchy.push( { subject: childClassmark, classmarkUrlSafe: childClassmark.split("class:")[1], prefLabel : hiddenLabel[0], hiddenLabel: hiddenLabel[0] } );
						}else{
							hierarchy.push(hiddenLabel[0]);
						}
					}
				...
				'@context': 'http://www.w3.org/2004/02/skos/core#',
  				'@graph': 
  					[ 
  						{ 
  							'@id': '_:b0', 
  							'changeNote':  new Date().getFullYear() + '-' + (new Date().getMonth().toString().length==1 ? '0' + new Date().getMonth().toString() : new Date().getMonth().toString()) + '-' + (new Date().getDate().toString().length==1 ? '0' + new Date().getDate().toString() : new Date().getDate().toString()) + ':Created:' 
  						} 
  					]
  				}
		...
		insertTriple: function(data,cb){

			if (!data){
				console.log("Bad triple data")
				if (cb) cb("Bad triple data",false)
				return false;
			}

			if (typeof data.subject == 'undefined' || typeof data.predicate == 'undefined' || typeof data.objectLiteral == 'undefined' || typeof data.objectUri == 'undefined' || typeof data.literalDataType == 'undefined' || typeof data.provenance == 'undefined' ){
				console.log("Missing triple data")
				if (cb) cb("Missing triple data",false)
				return false;
			}


			var args = [data.subject, data.predicate, data.objectUri, data.objectLiteral, data.literalDataType, data.provenance  ]



			this.argQuery('insert into triples (subject,predicate,objecturi,objectliteral,literaldatatype,provenance) values ($1,$2,$3,$4,$5,$6)',args,function(err, results) {
		...
		rebuildSearchLabel: function(subject,cb){

			var self = this
			var classmark = subject.replace('class:','');
			...
			self.returnClassmarkHierarchy(classmark,false,function(hierarchy){
				...
				self.returnClassmark(classmark,function(err,classmarkData){
					...
					pg.connect(process.env.DATABASE_URL, function(err2, client, done) {
						var queryText = 'update search set label = ($1) WHERE subject = ($2)'
						client.query(queryText, [label, subject], function(err, result) {
							if (err) console.log(err)
							if (result){
								if (result.rowCount==0){
									//there was no update performed, insert it
									var queryText = 'INSERT INTO search(subject, label, hiddenlabel, classification) VALUES($1, $2, $3, $4)'
									client.query(queryText, [subject, label, hiddenLabel, classification ], function(err, result) {
										if (err) console.log(err)
										done();
										if (cb)cb();
									});
