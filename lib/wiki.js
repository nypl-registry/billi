...
module.exports = function(app){
	...
	var wiki = {
		...
		process: function(classmark, wikidata, agent, cb, smallThumb){

			if (!smallThumb) smallThumb = false;
							...
							rdfExt.parseNTriples(body, function (graph,err) {
								...
								if (!err){
									...
									if (imageUrl){
										...
										commonsImageLink = "https://upload.wikimedia.org/wikipedia/commons/thumb/" + imageMd5.charAt(0) + '/' + imageMd5.charAt(0) + imageMd5.charAt(1) + '/' + encodeURI(imageSlug) + '/200px-' + encodeURI(imageSlug)



										if (smallThumb){
											imageUrl = imageUrl.replace("=300","=200");
											commonsImageLink = commonsImageLink.replace("200px-","100px-")
										}
												...
												.on('close', function(){
												  ...
												  fs.readFile('tmp_img', 'hex', function(err, imgData) {
												    ...
												    app.db.argQuery('insert into images (subject,image) values ($1,$2)',[classmark,imgData],function(err, writeResult) {
														...
														t5.objectUri = 'https://commons.wikimedia.org/wiki/File:' + imageSlug;
														t5.objectLiteral = null;
														t5.literalDataType = null;
														t5.provenance['@graph'][0].changeNote = t5.provenance['@graph'][0].changeNote + agent

														app.db.insertTriple(t5,function(err,results){

															//wait till this last one is done should be enough time 
															if (cb) cb(null,true)
